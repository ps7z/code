...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fail(X,Y,C,J,W) :- pathth(X,Y,C,J), indep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.
fail(X,Y,C,J,W) :- pathth(Y,X,C,J), indep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.
%fail(X,Y,C,J,W) :- pathhh(X,Y,C,J), indep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.
%fail(X,Y,C,J,W) :- pathtt(X,Y,C,J), indep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.

%fail(X,Y,C,J,W) :- not pathth(X,Y,C,J), not pathth(Y,X,C,J), not pathhh(X,Y,C,J), not pathtt(X,Y,C,J), dep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.
fail(X,Y,C,J,W) :- not pathth(X,Y,C,J), not pathth(Y,X,C,J), dep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.  

#minimize{W,X,Y,C,J:fail(X,Y,C,J,W) }.
