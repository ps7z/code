...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%fail(X,Y,C,J,M) :- th(X,Y,C,J), indep(X,Y,C,J,M,W), X<Y.
%fail(X,Y,C,J,M) :- th(Y,X,C,J), indep(X,Y,C,J,M,W), X<Y.
%fail(X,Y,C,J,M) :- hh(X,Y,C,J), indep(X,Y,C,J,M,W), X<Y.
%fail(X,Y,C,J,M) :- tt(X,Y,C,J), indep(X,Y,C,J,M,W), X<Y.


%:- not th(X,Y,C,J), not th(Y,X,C,J,M), not hh(X,Y,C,J), not tt(X,Y,C,J), dep(X,Y,C,J,W), not ismember(C,X), not ismember(C,Y),jset(J),cset(C), X<Y.

%#minimize{1,X,Y,C,J:fail(X,Y,C,J,M) }.
